...
public class GenerateNewNode : MonoBehaviour {
	...
	void Start() {
		Instance = this;
		parentGroup = GameObject.Find("/Main Game Scene/NodeGroup").transform;
	...
	public void Generate() {
		...
		for(int i = 0; i < GameControllerScript.Instance.nodeList.Count; i++) {
			if (GameControllerScript.Instance.nodeList [i].name == transform.name) {
				GameControllerScript.Instance.nodeList [i].setChildNodeName (childNames);
				break;
			}
		}
